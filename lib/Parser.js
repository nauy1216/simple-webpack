...
Parser.prototype.walkStatement = function (statement) {
    // type --> 语句类型 --> 比如： ExpressionStatement
    // expression --> 表达式 --> 比如： ExpressionStatement
...
Parser.prototype.walkExpression = function (expression) {
    ...
    switch (type) {
        ...
        case 'FunctionExpression':
            ...
            break;
        case 'CallExpression': // 调用方法
            let calleeName = [];
            let callee = expression.callee; // 调用的方法
            if (expression.arguments[0]) { // 调用参数
                this.walkExpression(expression.arguments[0]);// 对参数进行解析
            }
