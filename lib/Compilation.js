...
class Compilation extends Tapable {
    constructor(compiler) {
        ...
        this.mainTemplate = new MainTemplate(this.outputOptions);// 代码生成使用的主模板
        this.chunkTemplate = new JsonpChunkTemplate(this.outputOptions);// jsonp模板
        this.moduleTemplate = new ModuleTemplate(this.outputOptions, compiler.context); // 模块模板
        this.hotUpdateChunkTemplate = new HotUpdateChunkTemplate(this.outputOptions); // 热重载模板
        this.modules = [];
        this._modules = {};
        this.chunks = [];
        this.entries = [];
    ...
    _addModuleChain(context, dependency, onModule, callback) {
        const factory = this.dependencyFactories.get(dependency.class); // 每一种依赖都对应着一种创建模块的类
    ...
    addEntry(context, dependency, name, callback) {
        debugger
        const chunk = new Chunk({name}); // 每一个入口都会创建一个chunk
        this.chunks.push(chunk); // compilation.chunks 保存了所有的chunk
        chunk.chunks = this.chunks;
        this._addModuleChain(context, dependency, () => { //往chunk内添加module
    ...
    processModuleDependencies(module, callback) {
        ...
        // 处理模块依赖，先通过loaders解析当前模块, 然后parse代码字符串获取当前模块的依赖
